import * as skinview3d from "./src/skinview3d.js"
...
function reloadEars(skipSkinReload = false) {
  ...
  if (sourceType === "none") {
    ...
    hideInput = false
    document
      .querySelectorAll("#default_ears option[data-texture-type]")
      .forEach(opt => {
...
function initializeControls() {
  document
    ...
    .addEventListener("change", e => (skinViewer.width = e.target.value))
  document
    .getElementById("canvas_height")
    .addEventListener("change", e => (skinViewer.height = e.target.value))
  document
    .getElementById("fov")
    .addEventListener("change", e => (skinViewer.fov = e.target.value))
  document
    .getElementById("zoom")
    .addEventListener("change", e => (skinViewer.zoom = e.target.value))
  document
    .getElementById("global_light")
    .addEventListener(
      "change",
      e => (skinViewer.globalLight.intensity = e.target.value)
    )
  document
    .getElementById("camera_light")
    .addEventListener(
      "change",
      e => (skinViewer.cameraLight.intensity = e.target.value)
    )
  document
  ...
  document
    .getElementById("auto_rotate")
    .addEventListener("change", e => (skinViewer.autoRotate = e.target.checked))
  document
    .getElementById("auto_rotate_speed")
    .addEventListener(
      "change",
      e => (skinViewer.autoRotateSpeed = e.target.value)
    )
  for (const el of document.querySelectorAll(
    ...
    el.addEventListener("change", e => {
      document.getElementById(
        "crouch_setting"
      ).style.display = document.getElementById("animation_crouch").checked
        ? "block"
        : "none"
      if (e.target.value === "") {
        ...
        skinViewer.animation.speed = document.getElementById(
          "animation_speed"
        ).value
      }
    })
  ...
  const crouchSettings = {
    ...
    add_hit_animation: value => {
      document.getElementById("hit_speed_label").style.display = value
        ? "block"
        : "none"
  ...
  document
    ...
    .addEventListener(
      "change",
      e => (skinViewer.controls.enableRotate = e.target.checked)
    )
  document
    .getElementById("control_zoom")
    .addEventListener(
      "change",
      e => (skinViewer.controls.enableZoom = e.target.checked)
    )
  document
    .getElementById("control_pan")
    .addEventListener(
      "change",
      e => (skinViewer.controls.enablePan = e.target.checked)
    )
  ...
  document
    .getElementById("skin_url")
    .addEventListener("change", () => reloadSkin())
  document
    .getElementById("skin_model")
    .addEventListener("change", () => reloadSkin())
  document
    .getElementById("cape_url")
    .addEventListener("change", () => reloadCape())
  document
    .getElementById("ears_source")
    .addEventListener("change", () => reloadEars())
  document
    .getElementById("ears_url")
    .addEventListener("change", () => reloadEars())
  document
    .getElementById("panorama_url")
    .addEventListener("change", () => reloadPanorama())
  ...
  document
    .getElementById("nametag_text")
    .addEventListener("change", () => reloadNameTag())
}
...
function initializeViewer() {
  ...
  skinViewer.globalLight.intensity = document.getElementById(
    "global_light"
  ).value
  skinViewer.cameraLight.intensity = document.getElementById(
    "camera_light"
  ).value
  skinViewer.autoRotate = document.getElementById("auto_rotate").checked
  skinViewer.autoRotateSpeed = document.getElementById(
    "auto_rotate_speed"
  ).value
  const animationName = document.querySelector(
    'input[type="radio"][name="animation"]:checked'
  ).value
  if (animationName !== "") {
  ...
  skinViewer.controls.enableRotate = document.getElementById(
    "control_rotate"
  ).checked
  skinViewer.controls.enableZoom = document.getElementById(
    "control_zoom"
  ).checked
  skinViewer.controls.enablePan = document.getElementById("control_pan").checked
  for (const part of skinParts) {
    for (const layer of skinLayers) {
      skinViewer.playerObject.skin[part][
        layer
